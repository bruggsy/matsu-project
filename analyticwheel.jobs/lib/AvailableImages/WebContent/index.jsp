...
function initialize() {
    ...
    xmlhttp.open("GET", "GetPolygons", false);
    xmlhttp.send();
    data = JSON.parse(xmlhttp.responseText);

    hyperion = data["hyperion"];
    ali = data["ali"];
...
function drawData() {
	...
	for (var j in corners) {
	    coordinates.push(new google.maps.LatLng(corners[j][0], corners[j][1]));
	    sumlat += corners[j][0];
	    sumlng += corners[j][1];
	    sum1 += 1.0;
	}
	...
	if (inball  ||  radius == 0) {
		...
		return function() {
		    window.location.href = "index.jsp?centerLat=" + lat + "&centerLong=" + lng + "&radius=1";
		};
	    }(latitude, longitude));
